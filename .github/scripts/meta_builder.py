...
# This is the dict we'll store the catalog meta (things like categories)
catalog_meta = {
    "categories": []
}
...
for module_dir in os.listdir(modules_path):
    ...
    catalog[module_manifest['name']] = module_manifest

    category = str(module_manifest['category']).lower()
    if category not in catalog_meta.get("categories"):
        catalog_meta.get("categories").append(category)
...
catalog_artifact_path = f"{artifact_directory}/catalog.json"
os.makedirs(os.path.dirname(catalog_artifact_path), exist_ok=True)
f = open(catalog_artifact_path, "w")
f.write(json.dumps(catalog))
f.close()

# Make sure the path exists and write the manifest file
catalog_meta_artifact_path = f"{artifact_directory}/catalog-meta.json"
os.makedirs(os.path.dirname(catalog_meta_artifact_path), exist_ok=True)
f = open(catalog_meta_artifact_path, "w")
f.write(json.dumps(catalog_meta))
f.close()
